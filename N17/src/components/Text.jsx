...
export const Text = ({ store }) => {
  ...
  // Парсинг теста статьи
  useEffect(() => {
    const { current } = mainRef
    if (current) {

      // Поиск заголовков
      const recalculatedHeaders = () => {
        const headerElements = current.querySelectorAll('h3')
        const topics = Array.from(headerElements).map((header) => ({
            text: header.innerText,
            top: window.scrollY + header.getBoundingClientRect().top,
            element: header,
          })
        );
      ...
      Promise.all(promises).then(recalculatedHeaders);

      // Перепарсинг при изменении размеров
      document.addEventListener('resize', recalculatedHeaders)

      return () => document.removeEventListener('resize', recalculatedHeaders);
    }
  }, []);
  ...
  return (
    <main ref={mainRef}>
      ...
      <p>
        ...
        <strong>a new home page design and Smart Components</strong>, with <strong>Variants</strong> coming
        out of Beta. Looking at new features alone, it's very much alive and well.
      </p>
      <p>
      ...
      <div className="image">
        ...
        <div className="caption">Insert Menu within Framer</div>
      </div>
      ...
      <div className="image">
        <img
          src="./images/framer-community-plugins.gif"
          alt="Framer Community Plugins"
        />
        <div className="caption">Framer Community Plugins</div>
      </div>
      ...
      <div className="image">
        <img
          src="./images/framer-smart-components.gif"
